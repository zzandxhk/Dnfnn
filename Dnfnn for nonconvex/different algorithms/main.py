...
GRAPH_WIDTH = 25 # for a lattice graph
GRAPH_SIZE = 625 # 3x3
INPUT_DIM = 30
...
# def rand_edge(vi,vj,p=0.05):      #默认概率p=0.3
...
for i in tqdm.trange(episodes):
    ...
    if OBJECTIVE_F(init_x_4) - obj_F_min < eps:
        break
    next_x_3 = init_y_3 - ETA_1 * grad_F(init_y_3)
    next_v_3 = (1 - alpha) * init_v_3 + alpha * init_y_3 - ETA_1 / alpha * grad_F(init_y_3)
    next_y_3 = (next_x_3 + alpha * next_v_3) / (1 + alpha)
...
#ln2, = ax.plot(track_log_5, label='CGT-FR', marker='o', markersize=6, markevery=80)
ln1, = ax.plot(track_log_3, label='DGD', marker='v', markersize=6, markevery=160)
ln3, = ax.plot(track_log_6, label='EXTRA', marker='h', markersize=6, markevery=160)
ln4, = ax.plot(track_log_4, label='GT', marker='s', markersize=6, markevery=160)

plt.legend(handles=[ln1, ln3, ln4], loc='upper right')
plt.show()
